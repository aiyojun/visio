...
# 	tee /etc/apt/sources.list.d/realsense-public.list
...
echo ""
if [ -d /opt/visio ]; then
	echo "--> visio already in /opt, move to /opt/old_visio"
	echo ""
	mv /opt/visio /opt/old_visio
fi
cd /opt && git clone https://github.com/aa1608450902/visio.git && \
	cd visio && cp share/visio.desktop /usr/share/applications